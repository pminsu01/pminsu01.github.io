...
- ***2015.5.23 - 2015.8.22 : [나만의 비밀 일기장 애프터모먼트](https://github.com/pminsu01/Aftermoment)***
  - 기획 : 좋아하는 사람(또는 내 친구)의 기분은 어떨까?, SNS 형태의 일기장
  - 개발 : Front-End, 안드로이드 앱 개발(Android Studio 이용)
    - 회원가입, 로그인 로직 구현(Django 서버와 연동), 달력(GridViewLayout)에 나만의 이모티콘을 저장하는 로직 구현
...
- ***2016.8.20 - 2016.8.21  : [스몸비 방지 디바이스](https://github.com/pminsu01/SmartPhoneZombie)***
  ...
  - 개발 : Front-End, 안드로이드 앱 개발(Android Studio 사용)
...
- ***2017.3.2  - 2017.6.29 : [Where is my car?](https://github.com/pminsu01/WhereismyCar)***
  ...
  - 개발 : Full-Stack, 안드로이드 앱 화면 개발(Android Studio 사용), PHP 웹 서버 사용(Amazon EC2), 사진 저장을 위한 스토리지 서비스 사용(Amazon S3), 위치를 추적하여 사진을 찍는 디바이스 개발(Raspberrypi3)
    - H/W
      ...
      - 라즈베리파이3에서는 이와 동시에 Request 함수를 이용하여 사진을 openALPR 오픈소스를 사용하여 차량 번호 검출 로직, <br/>Amazon S3 버킷 저장 및 저장한 사진의 URL과 차량번호를 Amazon EC2의 Mysql DB에 저장하는 로직 구현(python)
    - S/W
      - H/W(Raspberrypi3)에서 전송하는 차량번호, 차량사진.jpg, 차량 사진 URL(S3 저장 정보) 정보를 Amazon EC2(O.S:Ubuntu) Mysql DB를 저장 및 관리(PHP)
      ...
      - 그 Bitmap과 차량 번호를 ListView에 적용하여 차량 사진과 차량 번호 안내할 수 있도록 개발 (MainUI를 바꾸기 위해 쓰레드와 핸들러를 모두 아우르는 AsyncTask로 구현)
